...
public class PolyglotBook {
    ...
    private Map<Long, EntrySet> book;

    /**
     * Creates a new Polyglot opening book using the data in the given file.
     *
     * @param filename The file of the Polyglot book to load.
    ...
    public PolyglotBook(String filename) throws IOException {
        ...
        while (infile.read(buffer, 0, ENTRY_SIZE) != -1) {
            ByteBuffer byteBuffer = ByteBuffer.wrap(buffer);

            Entry e = new Entry();

            long key = byteBuffer.getLong();
            e.move = byteBuffer.getShort();
            e.weight = byteBuffer.getShort() & 0xffff;

            // Decompose the raw move bitstring to get the move data
            e.toFile = e.move & 0b111;
            e.toRank = (e.move & 0b111000) >> 3;
            e.fromFile = (e.move & 0b111000000) >> 6;
    ...
    /**
     * Computes the Polyglot Zobrist hash for the given position.
     *
     * @param fen The FEN (or EPD) position to compute the Polyglot Zobrist hash
     *            for.
     * @return The Polyglot Zobrist hash for the given position.
     */
    public static long zobristHash(String fen) {
        long hash = 0;

        // Get relevant parts of the FEN string
        ...
        // Process piece positions and compute the piece component of the hash
        char[][] board = new char[BOARD_HEIGHT][BOARD_WIDTH];
        char[] piecesList = pieces.toCharArray();
        int pieceRank = BOARD_HEIGHT - 1;
        int pieceFile = 0;
        for (char c : piecesList) {
            switch (c) {
                case '8':
                case '7':
                case '6':
                ...
                case '/':
                    break;
                default:
                    int pieceOffset = NUM_SQUARES * PIECES.indexOf(c) + BOARD_WIDTH * pieceRank + pieceFile;
                    hash ^= RANDOM_64[pieceOffset];
                    board[pieceRank][pieceFile] = c;
                    pieceFile++;
            }
            if (pieceFile > BOARD_WIDTH - 1) {
                pieceFile = 0;
                pieceRank--;
            }
        }

        // Compute the turn component of the hash
        if (turn.charAt(0) == 'w') {
            hash ^= RANDOM_64[TURN_OFFSET];
        }

        // Compute the castling component of the hash
        if (castling.charAt(0) != ('-')) {
            for (int i = 0; i < castling.length(); i++) {
                switch (castling.charAt(i)) {
                    case 'K':
                        hash ^= RANDOM_64[CASTLE_OFFSET];
                        break;
                    case 'Q':
                        hash ^= RANDOM_64[CASTLE_OFFSET + 1];
                        break;
                    case 'k':
                        hash ^= RANDOM_64[CASTLE_OFFSET + 2];
                        break;
                    case 'q':
                        hash ^= RANDOM_64[CASTLE_OFFSET + 3];
                        break;
                }
            }
        }

        // Compute the en passant component of the hash
        if (enpassant.charAt(0) != '-') {
            int epFile = enpassant.charAt(0) - 'a';
            if (turn.charAt(0) == 'w') {
                // If it is white's turn, check if there is a white pawn that
                // can make an en passant capture
                if ((epFile > 0 && board[4][epFile - 1] == 'P') || (epFile < BOARD_WIDTH - 1 && board[4][epFile + 1] == 'P')) {
                    hash ^= RANDOM_64[EN_PASSANT_OFFSET + epFile];
                }
            } else {
                // If it is black's turn, check if there is a black pawn that
                // can make an en passant capture
                if ((epFile > 0 && board[3][epFile - 1] == 'p') || (epFile < BOARD_WIDTH - 1 && board[3][epFile + 1] == 'p')) {
                    hash ^= RANDOM_64[EN_PASSANT_OFFSET + epFile];
                }
            }
        }
    ...
    /**
     * Stores a set of book entries for some same position.
     */
    private static class EntrySet {
        final PriorityQueue<Entry> entries;

        /**
         * Creates a new, empty entry set.
         */
        public EntrySet() {
        ...
        /**
         * Adds a entry to this entry set.
         *
         * @param entry The entry to add to this set.
         */
        ...
        /**
         * Returns the entry in this set with the highest weight.
         *
         * @return The entry in this set with the highest weight, or null if
         * there are no entries in this set.
         */
        public Entry best() {
        ...
        /**
         * Returns a random entry in this set selected with a probability
         * proportional to its weight out of hte total weight of all entries in
         * this set.
         *
         * @return A random entry in this set selected with a probability
         * proportional to its weight, or null if there are no entries in this
         * set.
         */
        public Entry random() {
            int total = 0;
    ...
    /**
     * A single book entry for some position.
     */
    private static class Entry implements Comparable<Entry> {
         ...
         * @return A value such that when sorted, entries will be ordered by
         * descending weight.
         */
        @Override
