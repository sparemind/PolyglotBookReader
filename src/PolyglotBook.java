...
public class PolyglotBook {
     ...
     *
     * @param fen The position to compute the Polyglot Zobrist hash for.
    ...
    public static long zobristHash(String fen) {
        ...
        // Halfmove and Fullmove counts are ignored for this implementation
        ...
        for (char c : piecesList) {
            switch (c) {
                ...
                case '/':
                    break;
                default:
                    int pieceOffset = 64 * PIECES.indexOf(c) + 8 * pieceRank + pieceFile;
        ...
        if (enpassant.charAt(0) != '-') {
            ...
            if (turn.charAt(0) == 'w') {
                if ((epFile > 0 && board[4][epFile - 1] == 'P') || (epFile < 7 && board[4][epFile + 1] == 'P')) {
                    hash ^= RANDOM_64[EN_PASSANT_OFFSET + epFile];
                }
            } else {
                if ((epFile > 0 && board[3][epFile - 1] == 'p') || (epFile < 7 && board[3][epFile + 1] == 'p')) {
                    hash ^= RANDOM_64[EN_PASSANT_OFFSET + epFile];
                }
            }
        }
